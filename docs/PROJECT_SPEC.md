...
- Excluding inactive workers from schedule generation. The active flag is stored and editable; the Phase 6 eligibility logic now consumes it (an inactive worker is reported ineligible), but the Phase 7 schedule generator that would actually assign shifts does not exist yet and must apply the same exclusion, leaving historical assignments attached.
