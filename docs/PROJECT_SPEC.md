...
- Excluding inactive workers from schedule generation. The active flag is stored and editable; the Phase 6 eligibility logic consumes it (an inactive worker is reported ineligible). A read-only Phase 7 draft optimizer now exists and applies this same exclusion by reusing the Phase 6 eligibility logic directly, leaving historical assignments attached; it computes and returns a proposed draft but does not yet persist assignments. Supervisor approval, persistence, revalidation, replacement, and audit history remain planned scope.
