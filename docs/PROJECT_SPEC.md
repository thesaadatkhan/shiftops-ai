...
- Excluding inactive workers from schedule generation. The active flag is stored and editable; the Phase 6 eligibility logic consumes it (an inactive worker is reported ineligible). A Phase 7 draft optimizer computes a proposed draft, reusing the Phase 6 eligibility logic directly, leaving historical assignments attached. A supervisor must explicitly approve a stored proposal, through an explicit confirmation step naming exactly how many assignments will be created, before any assignment is created; approval revalidates every proposed assignment against current state and refuses the whole operation on any conflict, writing nothing partial. Each proposal persists an immutable coverage-review snapshot (existing/proposed assignments, coverage totals, uncovered reasons) at creation time, so it can be recovered and reviewed again - by week, newest first - after a refresh or navigation. The Schedule screen also lets a supervisor fill one uncovered position by choosing a currently eligible worker and explicitly confirming the write; this shares the same locked validation and audit primitive as agent-approved uncovered-shift proposals. An explicit replacement operation and an audit trail of assignment changes both exist. The Schedule frontend and shared reporting-week selector are implemented.

The database is the source of truth after explicit demo initialization. The canonical interview dataset is deterministic: 30 workers, confirmed and accepted semester coverage, recurring classes, preferences, approved leave and valid background assignments across the prepared weeks of September 21 and September 28, 2026. Deliberate covered, uncovered, constrained, call-out and replacement scenarios are hand-selected; only filler assignment selection uses a fixed random seed. Demo data is initialized once, into a database that holds no workforce or scheduling records, and that initialization is atomic: it either writes the whole dataset or nothing. If any such record already exists, initialization refuses and changes nothing, so it cannot overwrite edits, restore intentionally deleted workers, or add demo workers to a manually managed database. There is no repair mode, and neither application startup nor any employee-management action performs seeding. Replacing an established local demo database requires building and validating a separate file, stopping the application, preserving the current file under a timestamped non-overwritten backup, and only then swapping in the validated file. Schema changes are applied through repeatable migrations that preserve existing records and relationships. Most are purely additive; where a change must reshape existing data, it is carried out transactionally with the information preserved, never by resetting or reseeding the database. Provide an empty-workforce path for entering fictional workers through the interface. The 30-worker count, student-type mix, and course-load conventions constrain demo generation, not user-created records. The synthetic-only data policy and 20-hour weekly limit remain unchanged.
...
is shift eligibility itself - eligibility is now implemented as a separate
backend calculation (see Coverage below) that reads this readiness rather
than duplicating it.
...
Employees is a worker-management and summary view, not a full shift calendar. A shared reporting-week selector (previous/next week and a date picker), owned at the application level and used by both Employees and Schedule, is implemented (Phase 7 increment 4). It updates assigned hours and remaining weekly capacity for the selected week; it does not change stored worker identity, student type, active status, or records - selecting or reading a week never prepares shifts or mutates anything.
