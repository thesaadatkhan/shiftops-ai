...
- The AI scheduling agent's one remaining checklist item: a live-provider demonstration against a real `OPENAI_API_KEY` (see `.ai/DECISIONS.md` D054 for the exact command once a key is set). Everything else - backend foundation, approval/execution/verification, and the chat frontend - is implemented; see increments 1-3 above.
- Cloud deployment
...
Phase 9 (the AI scheduling agent) is functionally complete - backend
foundation, supervisor approval/execution/verification, and the AI
Assistant chat frontend are all built (see the bullet list below) - and
only the one required live-provider demonstration remains, pending a real
`OPENAI_API_KEY` being set in the environment.

Goals for the finished application are listed below. Employee records, shift preferences and approved leave are stored and editable, and the workforce summary and individual details are viewable. Semester and class editing, timetable confirmation, and preference and leave editing are all implemented. Deterministic eligibility/coverage and schedule evaluation/generation (optimizer draft, stored proposal, explicit approval, and explicit replacement) are also implemented. Dashboard and Workforce Planning analytics are implemented. The AI-agent goal below is functionally implemented; only its live-provider demonstration is outstanding.
